...
export const ParticlesOrb = forwardRef<HTMLDivElement, OrbProps>(function ParticlesOrb(
  {
    state = "idle",
    size = 168,
    speed = 1,
  ...
  useEffect(() => {
    ...
    const render = (dt: number, isStatic = false) => {
      ...
      const from: Rgb = isError ? ERROR_FROM_RGB : hexToRgb(colorRef.current.from);

      const to: Rgb = isError ? ERROR_TO_RGB : hexToRgb(colorRef.current.to);
